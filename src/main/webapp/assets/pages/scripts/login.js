            ...
            success: function (data, statusText, xhr, form) {
                if (data.code == 0) {
                   // $("#tip").hide();
                    ...
                    if($("#username").val() == "pqa")
                    	window.location.href = pathRoot+"/private/git/report";
                    else
                    	window.location.href = pathRoot+"/private/weekly/self";
                } else if (data.code == 2) {
                    $("#verifycode").val("");
                    $("#dialogVeryCode img").attr("src",pathRoot+'/private/login/captcha?_=' + new Date().getTime());
                    return $("#dialogVeryCode").modal({show: true, backdrop: 'static', keyboard: false});
                } else {
                	console.log(data.code);
                	 $('.alert-warning', $('.login-form')).show();
                    $("#captcha").val("");
/*                    $('#captcha_img').attr('src',pathRoot+'/private/login/captcha?_=' + new Date().getTime());
